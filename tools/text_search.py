...
def search_by_text_impl(
    ...
    """
    Search photos using a text query with optional metadata filters.

    Args:
        query: Text description of the photo content
    ...
    Returns:
        List of photo dictionaries with metadata and similarity scores
    """
    logger.info(f"Text search: query='{query}', filters={{year={year}, month={month}, time={time_of_day}}}, k={k}")
    ...
    try:
        ...
        # Generate embedding
        query_vec = embed_text(query, _model, _processor, _device)
...
search_by_text_tool = StructuredTool.from_function(
    func=search_by_text_impl,
    name="search_photos_by_text",
    description=(
